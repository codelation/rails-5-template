...
def add_active_storage
  insert_into_file 'Gemfile', 'gem "aws-sdk-s3"', after: /"codelation_assets"\n/
end
...
def setup_devise
  ...
  if @devise_user
    ...
    insert_into_file 'db/seeds.rb', "User.create!(email: 'user@codelation.com', password: 'password123', first_name: 'Jon', last_name: 'Doe') if Rails.env.development? && User.count.zero?"
  ...
  if @devise_admin_user
    ...
    insert_into_file 'db/seeds.rb', "AdminUser.create!(email: 'admin@codelation.com', password: 'password123') if Rails.env.development? && AdminUser.count.zero?"
  end
end
...
def setup_webpack
  ...
  run 'yarn upgrade'
  run 'yarn install stimulus'
  run 'yarn install rails-ujs'
end
